#include <optix_world.h> // Include access to the C++ APT in the optix namespaces.

using namespace optix;

rtDeclareVariable(float3, plane_normal, , );// The normal of this billboard
rtDeclareVariable(float3, plane_origin, , );// The origin of this billboard
rtDeclareVariable(float, radius, , );// The size of the billboard (Square - maybe this should be 2d to draw rectangles?)
rtDeclareVariable(int, circular, , ); // Whether this billboard is circular
rtDeclareVariable(int, lgt_instance, , ) = {0};

rtDeclareVariable(float3, precalc_xAxis, , ); // A place to store the pre-calculated xAxis vector
rtDeclareVariable(float3, precalc_yAxis, , ); // A place to store the pre-calculated yAxis vector
